...
- [ ] [2026-05-24] **P0 release risk: daily `Update Currency Rates` publish has failed continuously since 2026-05-12.** GitHub scout evidence: `gh -R firstbitelabsllc/resplit-currency-api run list --workflow 'Update Currency Rates' --limit 60` shows continuous failures through latest run `26354168265` (`2026-05-24T06:36:42Z`, head `68bbb436`). `gh run view 26350936742 --log-failed` showed generation reached `Snapshot window: 351 days (350 local, 0 network)` and wrote package files, then `validate-package` failed with `archive availableDates must contain at least 358 dates, got 351`. Root cause: validation blocks the daily snapshot commit before the workflow can recover from the May 12-23 publish hole; by May 24 the package has 351 dates and 14 archive gaps. Fix in this patch: `scripts/validate-package.js` keeps latest snapshot, exact 30-day history, ISO/span/shape checks, supported-currency, and artifact-presence checks hard, but warns on annual archive count/gap debt. The smoke gate is also hardened: `npm run smoke:deploy` now defaults to current UTC date and fails stale production instead of silently using the latest deployed stale date; `ALLOW_STALE_DEPLOY_SMOKE=1` is diagnostic-only. Proof: `node --test tests/smoke-check-deploy.test.js tests/validate-package.test.js` (12/12), `npm test` (115/115), and `npm run check` (generates May 24 package, validates with expected recovery warnings, 115/115 tests). Current deployed truth is still red: plain `npm run smoke:deploy` fails with `cloudflare latest date expected 2026-05-24, got 2026-05-11`; stale fallback passes only as `ALLOW_STALE_DEPLOY_SMOKE=1 npm run smoke:deploy`. Next: push/dispatch `Update Currency Rates`, let Actions commit the May 24 snapshot archive, rerun plain `npm run smoke:deploy`, and only then treat FX freshness as healthy in iOS/Web release confidence.
