...
- [x] [2026-07-18] **P1 cost: unchanged FX refreshes now avoid a duplicate public deployment train without weakening recovery.** The `00:00` and `03:00` UTC schedule remains intact, but the second pass checks the archive result, Cloudflare latest/history/meta plus dated snapshot, GitHub Pages fallback, exact Worker behavior, and trusted Worker release provenance before it skips Cloudflare Pages, Worker, GitHub Pages, and post-publish smoke. It accepts either the exact deployed SHA or a bot-authored archive-only child of that SHA after a complete parent diff proves no deployment input changed. Any stale/missing surface, invalid release provenance, validation error, archive change, or `workflow_dispatch` `force_publish=true` continues the normal publication path. Runtime secret validation/sync stays on every non-stale run, and `SENTRY_RELEASE` is stamped only after a successful Worker deployment so health cannot falsely represent a failed deploy. Workflow concurrency serializes overlapping passes. Source proof: Node 22 focused `65/65`, full pinned `PUBLISH_DATE=2026-07-18 npm run check` passed `587/587` Node plus `13/13` Worker tests, YAML parse, and `git diff --check`; read-only live probe for `2026-07-18` at `f290496b` returned `publish_required=false`, `publish_reason=verified_current`. No live workflow dispatch, Cloudflare mutation, secret write, or paid provider call occurred.
...
- [x] [2026-05-24] **P0 Worker liveness: `fx.resplit.app/health` is a dependency-free probe and deploy-smoke gate.** The Worker now serves GET/HEAD `/health` with `Cache-Control: no-store`, a stable `x-request-id`, release/environment metadata, and no archive/network dependency. `scripts/smoke-check-deploy.js` probes `/health` before quote/history/coverage checks so liveness regressions fail the same deploy gate that proves the FX data plane. Local proof before deploy: `node --test tests/fx-worker-routes.test.js tests/smoke-check-deploy.test.js`.

- [ ] [2026-05-24] **P1 release debt: archive recovery still has May 12-23 coverage gaps.** Publish is recovered, but `/coverage` still reports the known archive-only recovery gap for the latest 30-calendar-day window (`18/30` days available on 2026-05-24). Keep strict release validation/red history expectations until either May 12-23 snapshots are backfilled or those missing dates age out of the latest 30-day window. Do not relax stale/latest/fallback quote checks; the allowed deployed warning is only for current exact quotes with fresh latest data and the two known recovery signals. Backfill check: failed GitHub runs did not upload artifacts; `open.er-api.com` exposes latest-only data; Frankfurter has historical data but lacks AED; `@fawazahmed0/currency-api` historical snapshots cover the canary pairs but miss package currencies `clf`, `fok`, and `kid`. Do not commit partial/fabricated snapshots just to clear the canary.
