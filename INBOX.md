...
- [ ] [2026-05-25] **Grafana Cloud observability on Cloudflare Workers — prove first-party OTEL export.** Current Cloudflare guidance is first-party Workers Observability destinations, not Worker-side SDK wrapping. `wrangler.jsonc` should declare the top-level Worker config used by `.github/workflows/run.yml` (`npx wrangler deploy --config wrangler.jsonc --env=""`): logs destination `grafana-logs-prod`, traces destination `grafana-traces-prod`, 10% head sampling, and no secrets in source. **Non-goal**: replacing Sentry — `@sentry/cloudflare` stays for errors. **Trust gate**: after Cloudflare destinations exist and the Worker is redeployed, run `npm run observability:otel-smoke -- --since-minutes 60` with Grafana read env until both Tempo and Loki match in `reports/grafana-otel-smoke.json`. Config alone is yellow; only fresh Tempo+Loki proof can turn the cockpit green.
