...
The committed snapshot archive now retains FIVE CALENDAR YEARS of daily snapshots: everything dated on/after Jan 1 of (currentYear − 4) is kept, and strictly older dailies are pruned (a calendar-year boundary, not a fixed day count). **The five-calendar-year figure is a retention CAP, not available history: today the archive covers roughly one year, and it fills forward one day per publish.** The daily publish is FORWARD-ONLY by construction — `resolveProbeStartDate` pins the probe floor at the earliest snapshot already in the local archive, so a normal run can heal gaps INSIDE the archive span but can never add dates older than the existing edge (regression-tested across two consecutive publishes). Days pruned under the previous 365/400-day policies stay gone; deeper historical availability is deferred until an explicit provenance-approved backfill (the audited backfill contract below) is invoked deliberately — never fabricated or pulled implicitly from untrusted sources. One product-visible consequence of the calendar boundary: on each Jan 1 the window advances a full year at once, so receipts just past the horizon degrade from exact-date resolution to today-fallback overnight rather than gradually. Small archive gaps are tolerated and surfaced through `archive-manifest.json` / the coverage route rather than silently papered over. Before backfilling an archive gap, run `npm run audit:backfill-sources` and require at least one complete single-source candidate for every missing date; do not merge partial providers just to clear canary pairs.
